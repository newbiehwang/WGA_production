...
- **화면**: 위쪽 내비게이션의 '감사 로그' 탭(`/audit`)은 관리자에게만 보입니다(ID 토큰의 `cognito:groups`). 일반 사용자가 주소로 들어오면 홈으로 돌아갑니다. 탭을 숨기는 것은 편의이고, 실제로 막는 곳은 서버입니다. 화면 구성은 Datadog Audit Trail을 참고했습니다. 기간 안의 기록을 모두 받아(최대 2,000건), '필터' 버튼을 누르면 여는 창에서 종류·결과·층·요청자·도구·출처·표시를 값마다 건수를 보며 여러 개 함께 골라 거릅니다(고른 조건은 버튼 옆에 칩으로 보이고 ✕로 뺍니다). 건수는 다른 조건을 적용한 채 셉니다. 목록 위의 시간대별 막대그래프는 결과·종류·요청자·도구로 나눠 쌓아 보고(요청자·도구는 많은 순 5개와 기타, 범례를 누르면 그 값으로 거름), 드래그하거나 막대를 눌러 기간을 좁히며, 기간은 최근 1시간~30일 또는 직접 정한 구간(한국 시간)으로 고릅니다. 위쪽 검색창(`/`로 이동)은 요청자·도구·요약·입력값(리소스 ID)·질문·각종 ID에서 찾습니다(띄어 쓰면 모두 포함, "따옴표"는 한 덩어리, `-낱말`은 제외). 기간·거르기·검색어는 주소에 담겨 링크로 같은 화면을 나눌 수 있습니다. 행을 누르면 팝업창(대화 목록 팝업창과 같은 모양)에서 도구 입력·오류·질문 ID·가린 값의 수를 보이고, ↑/↓로 앞뒤 기록을 넘겨 보며, '같은 질문의 기록'·'같은 대화의 기록'·'같은 작업의 기록'·'이 요청자만'·'이 도구만'으로 이어 찾습니다(누르면 팝업창을 닫고 목록의 조건을 바꿉니다). '필터 초기화'는 처음 열었을 때의 화면으로 되돌립니다(거르기·검색어를 지우고, 기간은 최근 7일, 나눠 보기는 결과). 거르기와 건수는 브라우저에서 계산합니다(서버는 기록을 나눠 줄 뿐 세지 않습니다).
