...
- **화면**: 위쪽 내비게이션의 '감사 로그' 탭(`/audit`)은 관리자에게만 보입니다(ID 토큰의 `cognito:groups`). 일반 사용자가 주소로 들어오면 홈으로 돌아갑니다. 탭을 숨기는 것은 편의이고, 실제로 막는 곳은 서버입니다. 처음에는 모든 사용자의 기록이 보이고, 행을 누르면 도구 입력·오류·질문 ID·가린 값의 수를 펼쳐 봅니다.
...
- **역추적** (`GET /audit?trace=<actionId>&day=<YYYY-MM-DD>`, `services/llm/audit_trace.py`): 변경 작업 행을 펼쳐 '층별로 따져 보기'를 누르면, 그 작업을 놓고 아래에서 위로 묻습니다.
