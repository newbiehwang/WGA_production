...
- **역추적** (`GET /audit?trace=<actionId>&day=<YYYY-MM-DD>`, `services/llm/audit_trace.py`): 변경 작업 행의 팝업창을 열면 바로 그 작업을 놓고 7계층을 차례로 묻고, 답을 맨 위의 7계층 다이어그램에 얹습니다(누를 버튼이 없습니다). 제목 줄에 결론(가장 나쁜 판정의 색)과 이 변경을 낳은 질문을, 고리의 각 조각 안쪽 선에 그 계층의 판정(정상 초록·주의 주황·실패 빨강·참고 회색)을, 조각을 누르면 설명 칸에 그 계층의 물음·판정·답과 근거가 된 기록(시각·무엇)을 보입니다. 같은 작업의 요청·승인·실행 행은 모두 같은 역추적을 보입니다.
