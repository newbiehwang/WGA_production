...
- **피해 한정**: 탐지를 빠져나가도 변경 도구는 사람이 승인해야 실행되고(9번), IAM은 이 환경의 `wga-*`로 한정됩니다. 레드팀 테스트가 "모델이 로그 속 지시를 그대로 따라도 승인 없이는 바뀌지 않는다"를 확인합니다.
