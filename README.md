...
`dev:mock`은 `frontend/src/mock/api.ts`가 axios 요청을 가로채 백엔드와 같은 모양으로 응답합니다. 처음에는 예시 대화가 하나 있고, 질문을 보낼 때마다 도구 목록·표·목록·코드·실패한 도구가 담긴 예시 답변이 차례로 나옵니다. 대화 기록은 메모리에만 있어 새로 고치면 처음으로 돌아갑니다. mock 사용자는 관리자이고(사용자 관리 탭에 예시 사용자 5명), 주소에 `?mock-role=member`를 붙여 열면 일반 사용자 화면(감사 로그·사용자 관리 탭 없음)을 봅니다. 질문에 '천천히'를 넣으면 단계마다 여섯 배 느린 답변으로, 기다리는 동안의 한 줄이 바뀌어 가는 모습을 봅니다. 질문에 '로그대로'를 넣으면 로그에 심긴 지시를 따른 변경 요청(승인 카드의 의심 경고, 감사 로그의 체류)을 보고, 승인한 뒤 감사 로그에서 그 작업의 기록을 열면 7계층 다이어그램에서 역추적을 봅니다. 감사 기록은 실제 사용처럼 만듭니다(평일 업무 시간에 많고 밤·주말에 드묾, 도구가 많이 실패하는 장애 구간 세 번, 기록이 없는 하루, 30일에 약 1,000건). 역추적과 7계층 위치를 바로 확인하도록 판정이 서로 다른 변경 작업 여섯 가지(정상 승인·실행 `보존 기간 14일`, 거절 `알람 알림`, 실행 실패 `IncorrectInstanceState`, 의심 뒤 요청 `"의심 뒤 요청"`, 등록부에 없는 도구 `미등록 도구`, 승인 기록 없이 실행된 어긋난 기록 `퍼블릭 액세스`)와 사용자 관리 기록 세 건(초대·권한 변경·정지)도 기본 기록에 들어 있습니다(괄호 안은 검색어). 주소에 `?mock-audit=many`를 붙이면 약 2,700건을 만들어 긴 목록과 2,000건 한도를 확인합니다. 배포용 빌드에는 들어가지 않습니다.
