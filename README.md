...
- **층** (`locus`): 사고가 났을 때 어디가 뚫렸는지 좁히려고, 기록마다 도구 반복 위의 자리를 적습니다. fingate-x의 '원인의 계층'을 참고해 이 앱에 맞췄고, 판단은 바꾸지 않고 기록만 나눕니다.
  ...
  기록마다의 층은 아래 역추적의 7계층(효과·유출·체류·판단·유입·경계·매개) 중 기록으로 남길 수 있는 다섯입니다. 도구 호출·변경 작업의 팝업창 맨 위에는 7계층을 셰브런 조각 7개의 고리(순환 다이어그램)로 역추적과 같은 차례(맨 위 1 효과부터 시계 방향)로 그리고, 오른쪽 설명 칸에 이 기록이 한 일을 적습니다(조각을 누르면 그 계층의 설명, 제목 옆에 영어 이름). 제목은 '7계층 위치'입니다. 그 기록의 층(파랑으로 채우고 바깥으로 밀어냄)과 그 기록이 다른 층에 남긴 흔적(주황: 체류의 '의심 뒤 요청', 유입의 '의심 문구', 매개의 CloudTrail 요청 ID)을 짚습니다. 판단(모델 안)과 매개(앱 밖)는 점선 테두리입니다.
- **판정** (`GET /audit?trace=<actionId>&day=<YYYY-MM-DD>`, `services/llm/audit_trace.py`의 역추적): 변경 작업 행의 팝업창을 열면 바로 그 작업을 놓고 7계층을 차례로 따지고, 결과를 맨 위의 7계층 다이어그램에 얹습니다(누를 버튼이 없습니다). 제목 줄에 판정 개수(예: ● 주의 3 · ● 정상 3 · ● 참고 1)와 이 변경을 낳은 질문을, 고리의 조각 바탕색에 그 계층의 판정(정상 초록·주의 노랑·실패 빨강·참고 회색, 이때 이 기록의 계층은 번호의 파란 원)을, 조각을 누르면 설명 칸에 판정 배지와 그 계층에서 찾은 것, 근거가 된 기록(시각·무엇)을 보입니다. 서버가 계층마다 따지는 물음(아래 표)은 계층 정의와 겹쳐 화면에는 보이지 않습니다. 같은 작업의 요청·승인·실행 행은 모두 같은 판정을 보입니다.
