...
- **층** (`locus`): 사고가 났을 때 어디가 뚫렸는지 좁히려고, 기록마다 도구 반복 위의 자리를 적습니다. fingate-x의 '원인의 계층'을 참고해 이 앱에 맞췄고, 판단은 바꾸지 않고 기록만 나눕니다.
  ...
  기록마다의 층은 아래 판정에 쓰는 7계층(효과·유출·체류·판단·유입·경계·매개) 중 기록으로 남길 수 있는 다섯입니다. 도구 호출·변경 작업의 팝업창 맨 위 '7계층 위치'는 7계층을 고른 틈으로 나눈 고리로 그립니다(맨 위 효과부터 시계 방향). 한 가지 뜻에는 한 가지 표시만 씁니다: 조각의 바탕색은 이 작업의 판정(정상 초록·주의 노랑·실패 빨강·참고 회색, 판정이 없는 도구 호출 행은 옅은 중립색), 이름 위의 파란 점은 이 기록의 계층, 고리 바깥의 호는 고른 계층입니다. 조각을 누르면 오른쪽 카드가 그 계층으로 바뀝니다. 카드는 제목(번호·이름·영어 이름, 계층 정의는 ⓘ) 아래에 판정과 찾은 것(판정이 없으면 계층 정의), 이 기록(이 기록이 그 계층에 있는 까닭을 값의 흐름으로), 근거 기록(시각·무엇), 흔적(체류의 '의심 뒤 요청', 유입의 '의심 문구', 매개의 CloudTrail 요청 ID)을 가는 선으로 나눠 보입니다.
- **판정** (`GET /audit?trace=<actionId>&day=<YYYY-MM-DD>`, `services/llm/audit_trace.py`): 변경 작업 행의 팝업창을 열면 바로 그 작업을 놓고 7계층을 차례로 따져 보고(역추적), 결과를 '7계층 위치'에 얹습니다. 제목 줄 오른쪽에 판정 개수(예: 주의 3 · 정상 3 · 참고 1)와 이 변경을 낳은 질문, 조각의 바탕색에 계층마다의 판정, 카드에 그 계층에서 찾은 것과 근거 기록이 보입니다. 같은 작업의 요청·승인·실행 행은 모두 같은 판정을 보입니다. 서버가 계층마다 따지는 물음은 아래와 같습니다(화면에는 물음 대신 찾은 것만 보입니다).
...
`dev:mock`은 `frontend/src/mock/api.ts`가 axios 요청을 가로채 백엔드와 같은 모양으로 응답합니다. 처음에는 예시 대화가 하나 있고, 질문을 보낼 때마다 도구 목록·표·목록·코드·실패한 도구가 담긴 예시 답변이 차례로 나옵니다. 대화 기록은 메모리에만 있어 새로 고치면 처음으로 돌아갑니다. mock 사용자는 관리자이고(사용자 관리 탭에 예시 사용자 5명), 주소에 `?mock-role=member`를 붙여 열면 일반 사용자 화면(감사 로그·사용자 관리 탭 없음)을 봅니다. 질문에 '천천히'를 넣으면 단계마다 여섯 배 느린 답변으로, 기다리는 동안의 한 줄이 바뀌어 가는 모습을 봅니다. 질문에 '로그대로'를 넣으면 로그에 심긴 지시를 따른 변경 요청(승인 카드의 의심 경고, 감사 로그의 체류)을 보고, 승인한 뒤 감사 로그에서 그 작업의 기록을 열면 '7계층 위치'에서 판정을 봅니다. 감사 기록은 실제 사용처럼 만듭니다(평일 업무 시간에 많고 밤·주말에 드묾, 도구가 많이 실패하는 장애 구간 세 번, 기록이 없는 하루, 30일에 약 1,000건). 판정과 7계층 위치를 바로 확인하도록 판정이 서로 다른 변경 작업 여섯 가지(정상 승인·실행 `보존 기간 14일`, 거절 `알람 알림`, 실행 실패 `IncorrectInstanceState`, 의심 뒤 요청 `"의심 뒤 요청"`, 등록부에 없는 도구 `미등록 도구`, 승인 기록 없이 실행된 어긋난 기록 `퍼블릭 액세스`)와 사용자 관리 기록 세 건(초대·권한 변경·정지)도 기본 기록에 들어 있습니다(괄호 안은 검색어). 주소에 `?mock-audit=many`를 붙이면 약 2,700건을 만들어 긴 목록과 2,000건 한도를 확인합니다. 배포용 빌드에는 들어가지 않습니다.
