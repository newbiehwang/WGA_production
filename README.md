...
- **층** (`locus`): 사고가 났을 때 어디가 뚫렸는지 좁히려고, 기록마다 도구 반복 위의 자리를 적습니다. fingate-x의 '원인의 계층'을 참고해 이 앱에 맞췄고, 판단은 바꾸지 않고 기록만 나눕니다.
  ...
  기록마다의 층은 아래 역추적의 7계층(효과·유출·체류·판단·유입·경계·매개) 중 기록으로 남길 수 있는 다섯입니다. 도구 호출·변경 작업의 팝업창 맨 위에는 7계층을 원 위에 놓고 역추적과 같은 차례(① 효과부터)로 화살표를 이어 한 바퀴 그리며, 그 기록의 층(파랑, 가운데에 이름)과 그 기록이 다른 층에 남긴 흔적(주황: 체류의 '의심 뒤 요청', 유입의 '의심 문구', 매개의 CloudTrail 요청 ID)을 짚습니다. 판단(모델 안)과 매개(앱 밖)는 점선입니다.
