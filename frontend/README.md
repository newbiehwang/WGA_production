...
| `src/mock/api.ts` | mock 모드의 가짜 API (배포용 빌드에는 들어가지 않는다). 질문마다 몇 초짜리 진행 상황(사고 → 도구 → 사고)도 흉내 낸다. 감사 로그는 관리자 시점으로 세 사람의 30일치 예시를 두고, 질문을 보내면 기록이 추가된다. 질문에 '보존'(또는 '알람 … 꺼')이 들어 있으면 변경 작업 승인 흐름(승인 카드 → 승인·거절 → 결과 설명 → 감사 기록)을 흉내 낸다 |
