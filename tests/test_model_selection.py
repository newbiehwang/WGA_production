"""기본 모델 선택: 모델 ID를 고정하지 않고, 지금 제공되는 Sonnet 중 가장 낮은 버전을 고른다.
...
OLDEST_SONNET = "claude-sonnet-4-5-20250929"
...
def test_default_is_the_oldest_sonnet(llm):
    assert llm.pick_default_model(MODELS)["id"] == OLDEST_SONNET


def test_default_ignores_other_families_and_id_formats(llm):
    # ID 형식이 세대마다 달라도 출시일로 비교한다 (ID 문자열 순서로는 claude-3-…가 맨 앞에 온다)
    ...
    assert llm.pick_default_model(models)["id"] == "claude-sonnet-4-20250514"
...
def test_models_are_listed_across_all_pages(llm, monkeypatch):
    # 목록은 최신부터 오므로, 가장 오래된 Sonnet은 마지막 페이지에 있을 수 있다
...
@pytest.mark.parametrize("requested, expected", [
    (None, OLDEST_SONNET),                          # 요청이 없으면 기본 모델
    ("", OLDEST_SONNET),                            # 웹에서 목록을 받기 전 (빈 ID)
    ("claude-sonnet-5", "claude-sonnet-5"),         # 지금 제공되는 모델은 그대로
    ("claude-3-5-sonnet-20241022", OLDEST_SONNET),  # 퇴역한 모델 (예전 기본값이 저장돼 있던 경우)
])
...
def test_model_list_is_cached_and_survives_errors(llm, monkeypatch):
    ...
    assert llm.resolve_model_id(None) == OLDEST_SONNET
...
def test_health_reports_the_default_model(aws, monkeypatch):
    ...
    assert body["default_model"]["id"] == OLDEST_SONNET
